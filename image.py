...
A list of queries and their responses from a startup's pitch deck
Information about risks, barriers to entry, challenges, and competitors in the startup's market (gathered from internet research)

Your task is to:

Carefully analyze the CONTEXT provided.
Based on your analysis, provide comprehensive and in-depth recommendations to improve the startup's business plan. These recommendations should:

Address any weaknesses or gaps you identify in the current plan
Offer detailed, step-by-step guidance on how to implement improvements
Include specific examples, potential resources, or tools that could aid in implementation
Explain the rationale behind each recommendation and its potential impact
In your response, please:

Be extremely specific and actionable in your recommendations
Provide a balanced view, highlighting both positives and areas for improvement
Consider market dynamics, competitive landscape, and potential risks in depth
Suggest detailed ways to mitigate identified risks or overcome challenges
If applicable, propose alternative strategies or pivots that could enhance the startup's chances of success, including a thorough explanation of how to execute these alternatives
Offer insights on financial projections, marketing strategies, and operational improvements where relevant
Provide guidance on how to better articulate the startup's unique selling points and competitive advantages

Your goal is to provide comprehensive, valuable insights that will help the startup founder significantly refine their business plan and make their venture substantially more appealing to potential investors. Aim to give the founder a clear roadmap for improvement with actionable steps and detailed explanations.
Important formatting instruction:
Present your entire output in markdown format, but do not use any large headings (i.e., avoid using # or ## for headings). Use appropriate markdown formatting for emphasis, lists, and structure, but keep the overall presentation concise and without major section breaks.
...
def recommendations(queries, query_results, other_info_results):
    combined_results = {q: r for q, r in zip(queries, query_results)}
...
async def main(file_path, progress_callback=None):
    ...
    if progress_callback:
        progress_callback("Collecting other information...", 50)
